/* ============================================================
   vendaUltra.js — Final, pronto para Plano Ultra (5 anos)
   - Força slug: "plano_ultra_5_anos"
...
const PURCHASE_AMOUNT = 12500.0; // R$ — fallback
const PURCHASE_CURRENCY = "BRL";
const ASAAS_PIX_AMOUNT_BRL = 12500.0; // R$ — fallback
...
async function bootPaymentFor(method) {
  try {
    ...
    const desc = (inputDesc?.value || "Plano Ultra (5 Anos)").trim();
    ...
    // FORÇAR: garantir que esta página sempre envie ULTRA
    slug = "plano_ultra_5_anos";
    ...
    // LOG para debug — confirma o que vamos enviar
    ...
    // Reconhecer possíveis campos retornados contendo a imagem/base64 e o payload copia-cola
    const qrBase64 =
      data.qr_code_base64 ||
      data.encodedImage ||
...
async function onFormSubmit(e) {
  e.preventDefault();
  if (currentPaymentMethod === "pix") return; // Pix não passa por aqui
  ...
  try {
    ...
    const resp = await fetch(API_CREATE_PAYMENT_INTENT, {
      ...
      body: JSON.stringify({
        ...
        issuer_id: issuerId,
        description: "Plano Ultra (Cartão)",
        // inclui slug para backend
        plano: inputPlanoSlug?.value || "plano_ultra_5_anos"
      })
    });
...
/* ========================== Boot ========================== */
(function init() {
  // Forçar valores hidden corretos (segurança caso HTML esteja errado)
  try {
    if (inputDesc) inputDesc.value = "Plano Ultra (5 Anos)";
    if (inputPlanoSlug) inputPlanoSlug.value = "plano_ultra_5_anos";
    if (inputAmountCents && !inputAmountCents.value) inputAmountCents.value = "1250000";
  } catch (err) {
    console.warn("Não foi possível forçar valores hidden:", err);
  }

  // Colocar Pix como padrão (aba e painel)
