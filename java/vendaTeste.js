/* ============================================================
   vendaTeste.js — Final, pronto para Plano Teste (7 dias)
   - Força slug: "teste_7_dias"
...
const PURCHASE_AMOUNT = 1.0; // R$ — fallback
const PURCHASE_CURRENCY = "BRL";
const ASAAS_PIX_AMOUNT_BRL = 1.0; // R$ — fallback
...
async function bootPaymentFor(method) {
  try {
    ...
    if (!slug.startsWith("plano_") && slug !== "teste_7_dias") {
      slug = "teste_7_dias";
    }
  ...
  try {
    ...
    const resp = await fetch(API_CREATE_PAYMENT_INTENT, {
      ...
      body: JSON.stringify({
        ...
        issuer_id: issuerId,
        description: "Plano Teste (Cartão)",
        plano: inputPlanoSlug?.value || "teste_7_dias"
      })
    });
...
(function init() {
  try {
    ...
    if (inputAmountCents && !inputAmountCents.value) inputAmountCents.value = "100";
  } catch (err) {
