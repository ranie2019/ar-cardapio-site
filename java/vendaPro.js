/* ============================================================
   vendaPro.js — Final, pronto para Plano Pro (1 ano)
   - Força slug: "plano_pro_1_ano"
...
const PURCHASE_AMOUNT = 1200.0; // R$ — fallback
const PURCHASE_CURRENCY = "BRL";
const ASAAS_PIX_AMOUNT_BRL = 1200.0; // R$ — fallback
...
async function bootPaymentFor(method) {
  try {
    ...
    const desc = (inputDesc?.value || "Plano Pro (1 Ano)").trim();
    ...
    // FORÇAR: garantir que esta página sempre envie PRO
    slug = "plano_pro_1_ano";
  ...
  try {
    ...
    const resp = await fetch(API_CREATE_PAYMENT_INTENT, {
      ...
      body: JSON.stringify({
        ...
        issuer_id: issuerId,
        description: "Plano Pro (Cartão)",
        // inclui slug para backend
        plano: inputPlanoSlug?.value || "plano_pro_1_ano"
      })
    });
...
(function init() {
  ...
  try {
    if (inputDesc) inputDesc.value = "Plano Pro (1 Ano)";
    if (inputPlanoSlug) inputPlanoSlug.value = "plano_pro_1_ano";
    if (inputAmountCents && !inputAmountCents.value) inputAmountCents.value = "120000";
  } catch (err) {
